...
(
    ...
    foreign key (goz_attribute_id) references GozAttribute (goz_attribute_id) on delete no action,
    foreign key (genuine_project_id) references Project (genuine_project_id) on delete no action,
    foreign key (year_number) references Year (year_number) on delete no action,
